...
object community {
  def main(args: Array[String]): Unit = {
    ...
    var products = file.filter(_ != header).map(line => {
      val split = line.split(" ")
      (split(1).toInt, split(0).toInt)
    })
    var products1 = file.filter(_ != header).map(line => {
      val split = line.split(" ")
      (split(0).toInt, split(1).toInt)
    })
    products = products.union(products1)
    //println("PRODUCTS AFTER UNION")
    ...
    val edges = products.join(products).filter(x => x._2._1 != x._2._2).map { case (product, (useri, userj)) => ((useri, userj), 1) }
      .reduceByKey(_ + _).filter(_._2 >= 1).map(_._1)
  ...
  def maxModularity(graph: Map[Int, List[Int]], vertices: Set[Int], betweenness: Map[(Int, Int), Double]): (List[Set[Int]], Double) = {
    ...
    while (l < 42)
    {
      ...
      val deleteEdg = ListBuffer.empty[(Int, Int)]
      btw.foreach(x => {
          if (x._2 > i) {
            deleteEdg += x._1
          }
        })
      deleteEdg.foreach { case (v1, v2) => {
        newGraph.update(v1, newGraph(v1).filter(_ != v2))
        newGraph.update(v2, newGraph(v2).filter(_ != v1))
        btw -= Tuple2(v1, v2)
      }}
      maxCommunity = getCommunity(newGraph,vertices)
      l = maxCommunity.size
      i = i - 0.05
      println(i)
    }*/
